 ...
 */
$data = [
	'Этот', 'стиль', 'кода', 'часто', 'называют', 'Большой', 'Ком', 'Грязи', '(Big', 'Ball',
	'of', 'Mud).', 'Однако', 'в', 'этом', 'стиле', 'улучшение', 'стало', 'то,', 'что', 'заголовок',
	'и', 'нижний', 'колонтитул', 'веб-страницы', 'были', 'заключены', 'в', 'отдельные', 'файлы.',
];
$n = count($data);
$result = [];
for ($i = 0; $i < $n; $i++) {
	$result[$i] = preg_split('#[-\s,]+#s', $data[$i]);
}
print_r($result);
